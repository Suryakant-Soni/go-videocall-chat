...
func Stream(c *fiber.Ctx) error {
	...
	w.RoomsLock.Unlock()
	return c.Render("streams", fiber.Map{
		"NoStream": "true",
		"Leave":    "true",
	}, "layouts/main")
}
...
func StreamWebsocket(c *websocket.Conn) {
	log.Println("StreamWebsocket trigerred")
	suuid := c.Params("suuid")
	if suuid == "" {
		return
	}
...
func StreamViewerWebsocket(c *websocket.Conn) {
	log.Println("StreamViewerWebsocket trigerred")
	suuid := c.Params("suuid")
	if suuid == "" {
		return
	}
...
func viewerConn(c *websocket.Conn, p *w.Peers) {
	log.Println("viewerConn trigerred")
